...
function App() {
  ...
  const fetchPriceData = () => {
    fetch(`http://localhost:5000/prices/${ticker}`)
      .then(res => res.json())
      .then(setPriceData)
  ...
  useEffect(() => { fetchPriceData(); }, [ticker]);
  useEffect(() => {
  ...
  <div className="min-h-screen font-sans p-6 transition-colors bg-white dark:bg-gray-900 text-black dark:text-gray-100">
    <div className="max-w-7xl mx-auto">
      ...
      <div className="p-4 border rounded shadow bg-white dark:bg-gray-800 mb-6">
        ...
        <PriceChart ticker={ticker} data={priceData} />
      </div>

      {/* Filter Signals (moved below Price Chart) */}
      <section className="mb-6">
          ...
          <div>
            ...
            {['24h','7d','30d','90d','All'].map(r => (
              <button
                ...
                className={`px-3 py-1 rounded border mx-1 transition duration-200 hover:bg-blue-600 hover:text-white ${dateRange===r? 'bg-blue-700 text-white border-blue-800' : 'bg-white text-blue-600 border-blue-600'}`}
              >{r}</button>
            ))}
          </div>
          <div>
